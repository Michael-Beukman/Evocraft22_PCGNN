...
class MinecraftTown(Level):
    ...
    @staticmethod
    def random(width=10, height=1, depth=10) -> "MinecraftTown":
        level = MinecraftTown(width, height, depth)
        level.map = (np.random.randint(0, 4, size=level.map.shape)).astype(np.int32)
        return level

    
    @staticmethod
    def from_map(map: np.ndarray, **kwargs) -> "MinecraftTown":
        """Creates a level from an array representing the map.

        Returns:
            MazeLevel: 
        """
        level = MinecraftTown(map.shape[0], map.shape[1], map.shape[2], **kwargs)
        level.map = map
        return level
