...
class MoreSimpleDoorFitnessAgain(NeatFitnessFunction):
    def __init__(self, 
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def calc_fitness(level: Level):
            def has_door(single_face: np.ndarray):
                ...
                if len(counts) == 0: ONLY_TWO = 0
                elif min(counts) >= 2: ONLY_TWO = 1
                else: ONLY_TWO = 0
                ...
                return (how_many_things + how_many_good + ONLY_TWO) / 3
            ...
            ans = 0
            for k in [0, -1]:
                ans += dothings(M[:, -3:, k]) - dothings_bad(M[:, :-3, k])
                ans += dothings(M[k, -3:, :]) - dothings_bad(M[k, :-3, :])
                
                ans -= dothings_bad(M[:, k, :]) # subtract roof air
            ans /= 4
    ...
    def __repr__(self) -> str:
        return f"MoreSimpleDoorFitnessAgain(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
