...
class DecorationFitnessV9(NeatFitnessFunction):
    def __init__(self, 
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def calc_fitness(level: Level):
            ...
            if how_much_air == 1: return 0
            if how_much_air < 0.9: return 0
            how_much_air = np.clip(how_much_air / 0.95, 0, 1)
            how_many_torches = 1 - (M[1:-1, :, 1:-1] == 1).mean()
            
            temp = np.argwhere(M == 1) # shape of (num_torch, 3)
            if len(temp) == 0:
                DIST = 0
                ...
                # norm
                my_dist /= mycount
                my_dist /= np.sqrt(M.shape[0] ** 2 + M.shape[1] ** 2 + M.shape[2] ** 2)
            ...
            max_height = (M.shape[1] - np.argmax((M > 0), axis=1).min()) / M.shape[1]
            
            A = penalise_min_max((M == 1).sum(), 4, 8)
            B = penalise_min_max((M == 2).sum(), 1, 6 + 1)
            C = penalise_min_max((M == 3).sum(), 1, 2 + 1)
            D = penalise_min_max((M == 4).sum(), 1, 2 + 1)
            
            return (how_much_air + how_many_torches+ A + B + C + D + S1 + S2 + max_height + DIST) / 10
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
