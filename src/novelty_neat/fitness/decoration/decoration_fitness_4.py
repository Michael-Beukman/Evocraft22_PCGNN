...
class DecorationFitnessV4(NeatFitnessFunction):
    def __init__(self, 
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def penalise_min_max(a, mins, maxs):
            ...
            if a < mins: return a / max(mins, 1) / 4
            if a > maxs: return np.clip((a - maxs) / 20, 0, 0.4)
            return 0
        ...
        def calc_fitness(level: Level):
            ...
            if how_much_air > 0.98:
                how_much_air = 0.5
            else:
                how_much_air = np.clip(how_much_air / 0.95, 0, 1) ** 2
            ...
            A = penalise_min_max((M == 1).sum(), M.size // 200, 2 * M.size // 200)
            B = penalise_min_max((M == 2).sum(), 1, M.size // 200)
            C = penalise_min_max((M == 3).sum(), 1, M.size // 200)
            D = penalise_min_max((M == 4).sum(), 1, M.size // 200)
    ...
    def __repr__(self) -> str:
        return f"DecorationFitnessV4(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
