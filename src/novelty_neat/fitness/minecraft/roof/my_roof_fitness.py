...
class MyRoofFitness(NeatFitnessFunction):
    def __init__(self, 
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def calc_fitness(level: Level):
            ...
            how_many_tiles = (M > 0).sum() / M.size
            return (how_much_cover + how_much_connected + 1 - how_many_tiles) / 3
    ...
    def __repr__(self) -> str:
        return f"MyRoofFitness(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
