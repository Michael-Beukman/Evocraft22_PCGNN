...
class MyRoofFitnessV5(NeatFitnessFunction):
    def __init__(self, 
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def calc_fitness(level: Level):
            ...
            max_height = (M.shape[1] - np.argmax(M, axis=1).min()) / M.shape[1]
            
            return (how_much_cover + how_much_connected + 2*(1 - how_many_tiles) + max_height) / 5
    ...
    def __repr__(self) -> str:
        return f"MyRoofFitnessV5(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
