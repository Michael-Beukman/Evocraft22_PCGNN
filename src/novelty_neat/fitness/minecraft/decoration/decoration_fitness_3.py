...
class DecorationFitnessV3(NeatFitnessFunction):
    def __init__(self, 
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def calc_fitness(level: Level):
            ...
            A = penalise_min_max((M == 1).sum(), M.size // 400, 2 * M.size // 300)
            B = penalise_min_max((M == 2).sum(), 0, M.size // 300)
            C = penalise_min_max((M == 3).sum(), 0, M.size // 300)
            D = penalise_min_max((M == 4).sum(), 0, M.size // 300)
    ...
    def __repr__(self) -> str:
        return f"DecorationFitnessV3(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
