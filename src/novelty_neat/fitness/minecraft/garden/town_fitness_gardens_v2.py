...
class Gardens_FitnessV2(NeatFitnessFunction):
    def __init__(self, 
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def calc_fitness(level: Level):
            ...
            tree_score = 0
            if num_trees != 0:
                tree_score = 1 - ((num_bad_trees * num_bad_trees)/num_trees)
                if tree_score < -1:
                    tree_score = -1
            ...
            return (density_score + tree_score + water_score + has_tree + has_flowers + free_points) / 7
    ...
    def __repr__(self) -> str:
        return f"Gardens_FitnessV2(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
