...
class EmptyHouseFitness(NeatFitnessFunction):
    def __init__(self, 
                 number_of_levels_to_generate: int, 
                 level_gen: NeatLevelGenerator, square=None):
        """
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        def calc_fitness(level: Level):
            ...
            if self.square:
                return how_much_air_inside ** 2
            else:
                return how_much_air_inside
    ...
    def __repr__(self) -> str:
        return f"SimpleIsReasonableHouseFitness{'_Square' if self.square else ''}(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
